...
async function showRewardQueue(notices){
  ...
  try{
    for(const notice of notices){
      ...
      back.innerHTML=`<section class="reward-notice-card" role="dialog" aria-modal="true"><button class="reward-notice-close" aria-label="닫기">✕</button><div class="reward-notice-icon">🏆</div><h2>퀘스트 완료!</h2><p></p><div class="reward-chips"><span class="reward-chip">✨ +${Number(notice.xp||0)} XP</span><span class="reward-chip">🪙 +${Number(notice.gold||0)} G</span></div><button class="reward-confirm">보상 확인!</button></section>`;
  ...
  setTimeout(pollRewardNotices,50);
}
