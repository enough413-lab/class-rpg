...
shell.innerHTML=`<aside class="teacher-sidebar"><div class="teacher-brand">🌱 우리반 RPG<small>선생님 교실</small></div><nav aria-label="교사용 메뉴">${menus.map(([id,icon,label])=>`<button type="button" data-view="${id}"><span>${icon} ${label}</span><small id="nav-${id}"></small></button>`).join('')}</nav><a href="./" target="_blank" rel="noopener">학생 화면 ↗</a><button class="btn" onclick="logout()">로그아웃</button></aside><div class="teacher-content"><header class="teacher-heading"><div><p id="teacherDate" class="muted"></p><h1 id="teacherTitle" tabindex="-1">오늘의 현황</h1><p id="teacherSubtitle" class="muted"></p></div><button class="btn" id="dashboardRefresh">↻ 새로고침</button></header>${menus.map(([id])=>`<section id="view-${id}" class="teacher-view" aria-labelledby="teacherTitle" ${id==='today'?'':'hidden'}></section>`).join('')}</div>`;
...
for(const [view,icon,label] of requestTypes){const b=document.createElement('button');b.type='button';b.className='dashboard-request';b.innerHTML=`<span>${icon} ${label}</span><strong id="count-${view}">불러오는 중…</strong><small>확인하러 가기 →</small>`;b.onclick=()=>open(view,true);$('view-today').querySelector('.dashboard-requests').append(b)}
